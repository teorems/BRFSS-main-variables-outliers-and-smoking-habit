...
The first graph as a strongly positive skew distribution and the outliers are not showing at all (for example 1 entry with 45 people, one with 32). In fact households with more than 6 people are less than 0.01%. This kind of distribution could be adapted to be transformed with a logarithmic transformation in order to run parametric tests (e.g. for comparing two means). But this goes beyond the scope of this work. Or we could individuate and remove outliers, like we are going to do in the next section.
...
df2 %>%
  group_by(employ1) %>%
  summarise(meansleep = mean(sleptim1, na.rm = TRUE), mediansleep = median(sleptim1, na.rm = TRUE), min= min(sleptim1, na.rm = TRUE), max = max(sleptim1, na.rm = TRUE), sd = sd(sleptim1, na.rm= TRUE), count = n())
```
